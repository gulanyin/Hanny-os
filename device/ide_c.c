...
extern void register_extern_handler(int vector_number, void* _haneler);
...
static void select_disk(struct disk* hd) {
   ...
   if (hd->dev_no == 1) {	// device 寄存器  DEV 位 置1
...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
   ...
   // 写入要读写的扇区数, 如果sec_cnt为0,则表示写入256个扇区
...
static bool partition_info(struct list_elem* pelem, int arg ) {
   ...
   part = (struct partition *)( pop_block -  _offfset  );
...
void ide_init() {
    ...
    uint8_t hd_cnt = *((uint8_t*)(0x85004));	      // 获取硬盘的数量
    printk("hd_cnt is %u", hd_cnt);

    ASSERT(hd_cnt > 0);
    init_list(&partition_list);
    ...
    while (channel_no < channel_cnt) {
        ...
        printk("ide%u, %s", channel_no, channel->name);
        ...
        register_extern_handler(channel->irq_no, intr_hd_handler);

        while (dev_no < 1) {  // 只有一个硬盘
            struct disk* hd = &channel->devices[dev_no];
            hd->my_channel = channel;
            hd->dev_no = dev_no;
            sprintf(hd->name, "sd%c", 'a' + channel_no * 2 + dev_no);
            identify_disk(hd);	 // 获取硬盘参数
            // if (dev_no != 0) {	 // 当前只用了第一个通道的第一块硬盘
