...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
   ...
   uint32_t block_idx = 0;
   uint32_t* all_blocks = (uint32_t*)sys_malloc(512 + 48);	  // 12个直接块+128个间接块
   if (all_blocks == NULL) {
   ...
   while (block_idx < 140) {
      ...
      while (dir_entry_idx < dir_entrys_per_sec) {
    	 if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
    	    if (!strcmp((dir_e + dir_entry_idx)->filename, ".dsf")) {   // .dsf  .
    	       is_dir_first_block = true;
    	    } else if (strcmp((dir_e + dir_entry_idx)->filename, ".dsf") &&
    	       strcmp((dir_e + dir_entry_idx)->filename, "..dfw")) {
