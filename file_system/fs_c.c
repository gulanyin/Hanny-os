...
/* 在分区链表中找到名为part_name的分区,并将其指针赋值给cur_part */
 static bool mount_partition(PLIST_ELEM pelem, int arg) {
    char* part_name = (char*)arg;
    //struct partition* part = elem2entry(struct partition, part_tag, pelem);
    struct partition* part;
    int _offfset = (int)(&((struct partition*)0)->part_tag);
    int _start_addr = (int)pelem;
    part = (struct partition *)( _start_addr -  _offfset  );
    ...
    if (!strcmp(part->name, part_name)) {
        ...
        /**********     将硬盘上的块位图读入到内存    ****************/
        uint8_t* temp_add = (uint8_t *)sys_malloc(sb_buf->block_bitmap_sects * SECTOR_SIZE);
        if (temp_add == NULL) {
            PANIC("alloc memory failed!");
        }
        ...
        cur_part->block_bitmap.start_addr = temp_add;


        cur_part->block_bitmap.start_addr = (uint8_t*)sys_malloc(sb_buf->block_bitmap_sects * SECTOR_SIZE);
        if (cur_part->block_bitmap.start_addr == NULL) {
            PANIC("alloc memory failed!");
        }
        cur_part->block_bitmap.bytes_length = sb_buf->block_bitmap_sects * SECTOR_SIZE;
        /* 从硬盘上读入块位图到分区的block_bitmap.bits */
        ide_read(hd, sb_buf->block_bitmap_lba, cur_part->block_bitmap.start_addr, sb_buf->block_bitmap_sects);
        /*************************************************************/

        /**********     将硬盘上的inode位图读入到内存    ************/
        cur_part->inode_bitmap.start_addr = (uint8_t*)sys_malloc(sb_buf->inode_bitmap_sects * SECTOR_SIZE);
        if (cur_part->inode_bitmap.start_addr == NULL) {
            PANIC("alloc memory failed!");
        }
        cur_part->inode_bitmap.bytes_length = sb_buf->inode_bitmap_sects * SECTOR_SIZE;
        /* 从硬盘上读入inode位图到分区的inode_bitmap.bits */
        ide_read(hd, sb_buf->inode_bitmap_lba, cur_part->inode_bitmap.start_addr, sb_buf->inode_bitmap_sects);
        /*************************************************************/

        init_list(&cur_part->open_inodes);
        printk("mount %s done!\n", part->name);
...
static void partition_format(struct partition* part) {
    ...
    *************************************/
    ...
    printk("last_byte %u last_bit %u buf_size %u buf %x\n", block_bitmap_last_byte, block_bitmap_last_bit, buf_size, buf);
    ...
    ***************************************/
    ...
    /* 初始化当前目录"." */
    memcpy(p_de->filename, ".dsf", 4);
    p_de->i_no = 0;
    p_de->f_type = FT_DIRECTORY;
    p_de++;

    /* 初始化当前目录父目录".." */
    memcpy(p_de->filename, "..dfw", 5);
...
int32_t sys_write(int32_t fd, const void* buf, uint32_t count) {
   ...
   if (wr_file->fd_flag & O_WRONLY || wr_file->fd_flag & O_RDWR) {
      ...
      console_str("sys_write: not allowed to write file without flag O_RDWR or O_WRONLY\n");
      return -1;
   }
}
...
struct dir* sys_opendir(const char* name) {
   ...
   if (name[0] == '/' && (name[1] == 0 ||(name[0] == '.' && name[1] == 'd' && name[2] == 's' && name[3] == 'f'  )    )) {
      return &root_dir;
   }
...
int32_t sys_stat(const char* path, struct stat* buf) {
   /* 若直接查看根目录'/' */
   if (!strcmp(path, "/") || !strcmp(path, "/.dsf") || !strcmp(path, "/..dfw")) {
