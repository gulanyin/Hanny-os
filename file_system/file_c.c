...
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type) {
    ...
    uint32_t off_size = off_sec * 512;  // 本i结点索引相对于位图的字节偏移量
    uint32_t sec_lba;
    uint8_t* bitmap_off;
    ...
    switch (btmp_type) {
        case INODE_BITMAP:
            ...
            bitmap_off = part->inode_bitmap.start_addr + off_size;
            break;

        case BLOCK_BITMAP:
            sec_lba = part->sb->block_bitmap_lba + off_sec;
            bitmap_off = part->block_bitmap.start_addr + off_size;
            break;
    }
...
/* 同步内存数据到硬盘 */
   ...
   sys_free(io_buf);
   printk("create file %s success, inode_no is %u", filename, inode_no);
    ...
    if (flag & O_WRONLY || flag & O_RDWR) {	// 只要是关于写文件,判断是否有其它进程正写此文件
        ...
        if (!(*write_deny)) {    // 若当前没有其它进程写该文件,将其占用.
            *write_deny = true;   // 置为true,避免多个进程同时写此文件
            set_interrupt_status(old_status);	  // 恢复中断
        } else {		// 直接失败返回
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
   ...
   ASSERT(block_read_start_idx < 139 && block_read_end_idx < 139);  // 最大块数下标
